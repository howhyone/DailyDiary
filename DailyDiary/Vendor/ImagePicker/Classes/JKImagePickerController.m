...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        _toolbar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height-44-64, self.view.width, 44)];
        _toolbar.tintColor = [JKUtil getColor:@"f5f7fa"];
        if ([_toolbar respondsToSelector:@selector(barTintColor)]) {
            _toolbar.backgroundColor = [JKUtil getColor:@"f5f7fa"];
        }
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        NSLog(@"self.view.width is %f",self.view.width);
